 ...
 * - the offspring decision double-vector is instantiated using the parents' vectors (doubles).<br>
 *
 * @author MTomczyk
 */
...
{
    /**
     * Auxiliary interface for reproducing the specimens. This reproducer constructs one offspring from two parents.
     */
    public interface IReproduce
    {
         ...
         *
         * @param p1 the first parent (decision double-vector; reference; do not modify it)
         * @param p2 the second parent (decision double-vector; reference; do not modify it)
    ...
    public interface IReproduceTO
    {
         ...
         * is the second element; decision double-vectors)
         */
        double[][] reproduce(double[] p1, double[] p2, IRandom R);
    }
    ...
    /**
     * Auxiliary object for reproducing specimens (if provided, it constructs one offspring using two parents).
     */
    private final IReproduce _reproducer;

    /**
     * Auxiliary object for reproducing specimens (if provided, it constructs two offspring using two parents).
     */
     ...
     */
    private DoubleReproduce(IReproduce reproducer, IReproduceTO reproducerTO, IReproduceMO[] reproducersMO)
    {
     ...
     */
    public static DoubleReproduce getInstanceTO(IReproduceTO reproducer)
    {
        return new DoubleReproduce(null, reproducer, null);
    }
     ...
     * @return object instance
     */
    public static DoubleReproduce getInstanceMO(IReproduceMO[] reproducersMO)
    {
        return new DoubleReproduce(null, null, reproducersMO);
    }
     ...
     * @param reproducersMO auxiliary objects for reproducing specimens (constructs an arbitrary number of offspring
     *                      solutions using two parents); if different reproducers sharing the same
     *                      {@link DoubleReproduce.IReproduceMO#getNoOffspring()} are provided, only the last one will
     *                      be used (the reproducers are supposed to differ in this regard)
     * @return object instance
     */
    public static DoubleReproduce getInstance(DoubleReproduce.IReproduce reproducer,
                                              DoubleReproduce.IReproduceTO reproducerTO,
                                              DoubleReproduce.IReproduceMO[] reproducersMO)
    {
        return new DoubleReproduce(reproducer, reproducerTO, reproducersMO);
    }


    /**
     * The main method that creates the offspring. The reproduction is delegated to an auxiliary object implementing
     * the {@link IReproduce}, {@link IReproduceTO}, or {@link IReproduceMO}. The method uses this reproducer that
     * matches {@link Parents#_noOffspringToConstruct} value (determined for each Parents object). If no matching
    ...
    public ArrayList<Specimen> createOffspring(IEA ea) throws PhaseException
    {
        ...
        for (Parents p : ea.getSpecimensContainer().getParents())
        {
            ...
            } else if (p._noOffspringToConstruct == 2)
            {
                ...
                double[][] o = _reproducerTO.reproduce(
                        p._parents.get(0).getDoubleDecisionVector(),
                        p._parents.get(1).getDoubleDecisionVector(),
                        ea.getR());
            ...
            } else if ((_reproducersMO != null) && (_reproducersMO.containsKey(p._noOffspringToConstruct)))
            {
                ...
                double[][] o = reproduceMO.reproduce(
                        p._parents.get(0).getDoubleDecisionVector(),
                        p._parents.get(1).getDoubleDecisionVector(),
                        ea.getR());
                ...
                {
                    Specimen s = new Specimen(ea.getCriteria()._no);
                    s.setChromosome(new Chromosome(doubles));
                    offspring.add(s);
                }
            } else
            {
