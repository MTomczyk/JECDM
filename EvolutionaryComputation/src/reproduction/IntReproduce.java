 ...
 * - the offspring decision int-vector is instantiated using the parents' vectors (integers).<br>
 *
 * @author MTomczyk
 */
public class IntReproduce implements IReproduce
{
    ...
    public interface IReproduce
    {
         ...
         *
         * @param p1 the first parent (decision int-vector; reference; do not modify it)
         * @param p2 the second parent (decision int-vector; reference; do not modify it)
    ...
    public interface IReproduceTO
    {
         ...
         * is the second element; decision int-vectors)
         */
        int[][] reproduce(int[] p1, int[] p2, IRandom R);
    }
     ...
     */
    private IntReproduce(IReproduce reproducer, IReproduceTO reproducerTO, IReproduceMO[] reproducersMO)
    {
     ...
     */
    public static IntReproduce getInstanceTO(IReproduceTO reproducer)
    {
        return new IntReproduce(null, reproducer, null);
    }
     ...
     * @return object instance
     */
    public static IntReproduce getInstanceMO(IReproduceMO[] reproducersMO)
    {
        return new IntReproduce(null, null, reproducersMO);
    }
     ...
     * @return object instance
     */
    public static IntReproduce getInstance(IReproduce reproducer, IReproduceTO reproducerTO, IReproduceMO[] reproducersMO)
    {
        return new IntReproduce(reproducer, reproducerTO, reproducersMO);
    }
    ...
    public ArrayList<Specimen> createOffspring(IEA ea) throws PhaseException
    {
        ...
        for (Parents p : ea.getSpecimensContainer().getParents())
        {
            ...
            } else if ((_reproducersMO != null) && (_reproducersMO.containsKey(p._noOffspringToConstruct)))
            {
                ...
                {
                    Specimen s = new Specimen(ea.getCriteria()._no);
                    s.setChromosome(new Chromosome(integers));
                    offspring.add(s);
                }
            } else
            {
