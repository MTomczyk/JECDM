...
/**
 * Provides various tests for {@link NEMO0}
 *
 * @author MTomczyk
 */
class NEMO0Test
{
    ...
    void testPerformedEvaluations()
    {
                ...
                {100, 10, 40, 25}, {30, 50, 20, 25}, {30, 50, 20, -5}})
        {
            ...
            DSSB.setPreferenceModel(new model.definitions.LNorm());
            nemo0Builder.setFixedOSBoundsLearningPolicy(problemBundle._normalizations, problemBundle._utopia, problemBundle._nadir);
            nemo0Builder.setEAParamsAdjuster(p -> {
    ...
    void testOSManager()
    {
        for (int mode = 0; mode < 10; mode++)
        {
            ...
            nemo0Builder.setPopulationSize(3);
            nemo0Builder.setCriteria(C);
            nemo0Builder.setParentsSelector(new Random(2));
            nemo0Builder.setInitialPopulationConstructor(new DoubleConstruct(R -> new double[0])); // dummy constructor
            nemo0Builder.setParentsReproducer(new DoubleReproduce((p1, p2, R) -> new double[0])); /// dummy reproducer
            ...
            nemo0Builder.setSpecimensEvaluator(new DoubleEvaluate(v -> evaluations.pop()));
            ...
            if (mode == 0)
                nemo0Builder.setFixedOSBoundsLearningPolicy(new INormalization[]{new Linear(), new Linear()});
            else if (mode == 1) nemo0Builder.setFixedOSBoundsLearningPolicy(new INormalization[]{new Linear(),
                    new Linear()}, new double[]{1.0d, 2.0d}, null);
            else if (mode == 2) nemo0Builder.setFixedOSBoundsLearningPolicy(new INormalization[]{new Linear(),
                    new Linear()}, new double[]{1.0d, 2.0d}, new double[]{5.0d, 10.0d});
            else if (mode == 3)
            {
                nemo0Builder.setDynamicOSBoundsLearningPolicy();
                nemo0Builder.setOSMParamsAdjuster(p -> p._listeners = new IOSChangeListener[]{
                        iosChangeListener
                });
            }
            else if (mode == 4)
            {
                nemo0Builder.setDynamicOSBoundsLearningPolicy(new INormalization[]{new Linear(), new Linear()});
                nemo0Builder.setOSMParamsAdjuster(p -> p._listeners = new IOSChangeListener[]{
                        iosChangeListener});
            }
            else if (mode == 5)
            {
                nemo0Builder.setDynamicOSBoundsLearningPolicy(new INormalization[]{
            ...
            else if (mode == 6)
            {
                nemo0Builder.setDynamicOSBoundsLearningPolicy(new INormalization[]{
                        new Linear(), new Linear()}, new double[]{100.0d, -100.0d}, new double[]{-100.0d, 100.0d});
                nemo0Builder.setOSMParamsAdjuster(p -> {
            ...
            try
            {
                NEMO0 nemo0 = nemo0Builder.getInstance();
                IRunner runner = new Runner(nemo0);
                runner.executeEvolution(2);

                if (mode == 0)
                {
                    assertNull(nemo0.getObjectiveSpaceManager());
                ...
                {
                    assertNotNull(nemo0.getObjectiveSpaceManager());
                    TestUtils.assertEquals(new double[]{1.0d, 2.0d}, nemo0.getObjectiveSpaceManager().getOS()._utopia, 1.0E-6);
                    TestUtils.assertEquals(new double[]{5.0d, 10.0d}, nemo0.getObjectiveSpaceManager().getOS()._nadir, 1.0E-6);
                    assertNull(osChangedReport._previousOS);
                    assertNull(osChangedReport._currentOS);
                }
                else if (mode == 3)
                {
                    assertNotNull(nemo0.getObjectiveSpaceManager());
                    TestUtils.assertEquals(new double[]{2.0d, -1.0d}, nemo0.getObjectiveSpaceManager().getOS()._utopia, 1.0E-6);
                    TestUtils.assertEquals(new double[]{-1.0d, 2.0d}, nemo0.getObjectiveSpaceManager().getOS()._nadir, 1.0E-6);
                ...
                else if (mode == 6)
                {
                    assertNotNull(nemo0.getObjectiveSpaceManager());
                    TestUtils.assertEquals(new double[]{2.0d, -1.0d}, nemo0.getObjectiveSpaceManager().getOS()._utopia, 1.0E-6);
                    TestUtils.assertEquals(new double[]{-100.0d, 100.0d}, nemo0.getObjectiveSpaceManager().getOS()._nadir, 1.0E-6);
                ...
                {
                    assertNotNull(nemo0.getObjectiveSpaceManager());
                    TestUtils.assertEquals(new double[]{100.0d, -100.0d}, nemo0.getObjectiveSpaceManager().getOS()._utopia, 1.0E-6);
                    TestUtils.assertEquals(new double[]{-1.0d, 2.0d}, nemo0.getObjectiveSpaceManager().getOS()._nadir, 1.0E-6);
                ...
                else if (mode == 8)
                {
                    ...
                    if (mode == 9)
                    {
                        assertNotNull(nemo0.getObjectiveSpaceManager());
                        TestUtils.assertEquals(new double[]{2.0d, -1.0d}, nemo0.getObjectiveSpaceManager().getOS()._utopia, 1.0E-6);
                        TestUtils.assertEquals(new double[]{0.0d, 0.5d}, nemo0.getObjectiveSpaceManager().getOS()._nadir, 1.0E-6);
