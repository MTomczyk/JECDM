...
/**
 * Several tests (visualizations) for the SBX-TO crossover operator
 *
 * @author MTomczyk
 */
public class SBXTOTest
{
    /**
    ...
    public static void main(String[] args) throws RuntimeException
    {
        ...
        for (int value = 0; value < values.length; value++)
        {
            ...
            for (int index = 0; index < distributionIndexes.length; index++)
            {
                ...
                SBXTO.Params pSBX = new SBXTO.Params(1.0d, distributionIndexes[index]);
                pSBX._valueCheck = new Wrap();

                SBXTO sbx = new SBXTO(pSBX);

                for (int t = 0; t < trials; t++)
                {
                    double[] p1 = new double[]{values[value][0]};
                    double[] p2 = new double[]{values[value][1]};
                    double[] o1 = sbx.crossover(p1, p2, R)._o1;
                    double[] o2 = sbx.crossover(p1, p2, R)._o2;
                    DD.add(o1[0]);
                    DD.add(o2[0]);
                }

                assert DD.getNoSamples() == trials * 2;
