 ...
 * @author MTomczyk
 */
class VisualizationTest_NoTimesReached
{
    /**
    ...
    public static void main(String[] args)
    {
        ...
        pHP._featureGetter = new ConstantValue(1.0d);
        pHP._trialStatistics = new Min();
        pHP._finalStatistics = new Sum();
        ...
        pPlot._heatmapDisplayRange = new DisplayRangesManager.DisplayRange(new Range(0.0d, trials), false, false);
        pPlot._gradient = Gradient.getPlasmaGradient();
        ...
        pPlot._colorbar = new Colorbar(Gradient.getPlasmaGradient(), "Expected generation-no when occupied", new FromDisplayRange(pPlot._heatmapDisplayRange, 5));

        Heatmap3D HM = new Heatmap3D(pPlot);
        HM.getComponentsContainer().getMargins().getSurpassedSizes().put(SizeFields.MARGIN_RIGHT_RELATIVE_SIZE_MULTIPLIER, 0.3f);
