 ...
 * comprehensive rank-based summaries (concerning results attained in the last generation). In this case, it is required
 * to specify a key whose values will be explicitly compared against one another (selected key). The example and default
 * implementations are {@link FinalRankerXLS} and {@link FinalStatisticsXLSX}. They store the final results in tables;
 * each one is stored in a separate sheet, and each one is dedicated to a different performance indicator. The
 * dimensionality N of the cross-analysis can be specified. All crossed key-values (cartesian products) are provided
 * on the left side of the table. Note that the selected kay is always pushed to the right for better visibility. The
 * next columns are devoted to the rank analysis: wins matrix, tries matrix, and ranks. Regarding the wins matrix, the
 * trial results attained for the selected key when using different values are directly compared (one by one, i.e., the
 * first trial result against the first trial result, and so on). It is counted how often one scenario provided better
 * results than the other yielded scenario (the preference direction is provided via the indicator used). Similarly,
 * the procedure counts the number of ties (the tolerance level can be provided). Next, the ranks are calculated. In
 * this case, for the selected key, it is counted how many times each value attained the best results (rank of 1), the
 * second best (rank of 2), and so on, when compared against other values (again, trial results are compared one by one).
 * In the case of a tie, the rank distribution is spread among similar performers. E.g., if two values attain the best
 * results (tie), they receive a 0.5 bonus for ranks 1 and 2 (instead of a bonus of 1 for just one rank). The next best
 * performer is then associated with a rank of 3. Additionally, the average rank is provided (1 is the best). Lastly,
 * statistical tests implementing {@link statistics.tests.ITest} can be provided via this constructor. For each provided
 * test, an additional block of results directly comparing one value against another is supplied with the produced tables.
 *
 * @author MTomczyk
 */
public abstract class AbstractExcelFinalRanker extends AbstractExcelSaver implements ICrossSaver
{
    /**
    ...
    /**
     * Joint key indexes (indexes of all keys except the selected + index of the selected at the end; refer to the original indexing).
     */
    private int[] _jointKeyIndexes;

    /**
     * Auxiliary map that transforms a joint ket in {@link AbstractExcelFinalRanker#_jointKeyNames} into their in-array indexes.
     */
     ...
     */
    public AbstractExcelFinalRanker(String path, String filename, CrossedScenarios crossedScenarios,
                                    String selectedKey,
                                    ITest[] tests,
    ...
    protected void instantiateTables()
    {
        ...
        for (int i = 0; i < _tables.length; i++)
        {
            ...
            _tables[i].applyChessboard(2 * (_level), 2, cbs, rbs, _excel._style._tableFillChessboardColor);
