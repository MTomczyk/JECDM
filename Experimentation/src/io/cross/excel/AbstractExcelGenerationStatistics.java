 ...
 * summarize the final results (or results attained in some specified generation). The example and default
 * implementations are {@link FinalStatisticsXLS} and {@link FinalStatisticsXLS}. They store the final results in tables;
 * each one is stored in a separate sheet, and each one is dedicated to a different performance indicator.
 * The dimensionality N of the cross-analysis can be specified. The first N - 1 key-values objects are linked to tables'
 * rows, while the last key-values object is associated with columns.
 *
 * @author MTomczyk
 */
public abstract class AbstractExcelGenerationStatistics extends AbstractExcelSaver implements ICrossSaver
{
     ...
     */
    public AbstractExcelGenerationStatistics(String path,
                                             String filename,
                                             CrossedScenarios crossedScenarios,
                                             int level,
                                             Style style)
    {
        this(path, filename, crossedScenarios, null, level, style);
    }
    ...
    protected void instantiateTables()
    {
        ...
        for (int i = 0; i < _tables.length; i++)
        {
            ...
            newStyle.cloneStyleFrom(_tables[i]._cell[3][2 * (_level - 1)].getCellStyle());
            _tables[i].applyChessboard(2 * (_level - 1), 3,
                    cbs, rbs, newStyle, _excel._style._tableFillChessboardColor);
        }
    }
