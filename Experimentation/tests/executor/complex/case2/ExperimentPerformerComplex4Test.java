 ...
 * @author MTomczyk
 */
class ExperimentPerformerComplex4Test
{
    ...
    void test()
    {
        ...
        String msg = null;
        try {
            folderPath = FileUtils.getPathRelatedToClass(ExperimentPerformerComplex4Test.class,
        ...
        // TXT LOADING load data and compare the results
        {
        ...
        // CSV LOADING load data and compare the results
        {
            String[] txtResults = new String[]{
                    mainPath + File.separatorChar + "PS_3_GEN_10" + File.separatorChar + "PS_3_GEN_10.csv",
                    mainPath + File.separatorChar + "PS_2_GEN_5" + File.separatorChar + "PS_2_GEN_5.csv"
            };
        ...
        // EXCEL LOADING load data and compare the results
        {
            String[] excelResults = new String[]{
                    mainPath + File.separatorChar + "PS_3_GEN_10" + File.separatorChar + "PS_3_GEN_10.xls",
                    mainPath + File.separatorChar + "PS_3_GEN_10" + File.separatorChar + "PS_3_GEN_10.xlsx",
                    mainPath + File.separatorChar + "PS_2_GEN_5" + File.separatorChar + "PS_2_GEN_5.xls",
                    mainPath + File.separatorChar + "PS_2_GEN_5" + File.separatorChar + "PS_2_GEN_5.xlsx"
            };

            int[] sheets = new int[]{2, 2, 1, 1};
            double[][][][] res = new double[][][][]
                    {
                            {ExpectedResults._t4ps3gen10e3_min0, ExpectedResults._t4ps3gen10e3_sum1},
                            {ExpectedResults._t4ps2gen5e3_mean2_full},
            ...
            for (int f = 0; f < 4; f++) {
                ...
                for (int sheet = 0; sheet < sheets[f]; sheet++) {
                    Sheet sh = workbook.getSheetAt(sheet);
                    double[][] exp;
                    if (f < 2) exp = res[0][sheet];
                    else exp = res[1][sheet];

                    for (int gen = 0; gen < exp.length; gen++) {
                        Row row = sh.getRow(1 + gen);
                        for (int j = 0; j < exp[0].length; j++) {
                            Cell cell = row.getCell(1 + j);
                            double v = cell.getNumericCellValue();
                            assertEquals(exp[gen][j], v, 0.0000001d);
                        }
