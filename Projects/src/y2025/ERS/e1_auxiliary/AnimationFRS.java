 ...
 * @author MTomczyk
 */
@SuppressWarnings({"DuplicatedCode", "ConstantValue", "BusyWait"})
public class AnimationFRS
{
    ...
    public static void main(String[] args) throws InterruptedException, IOException
    {
        ...
        // FRS sampler
        IterableFRS.Params<LNorm> pFRS = new IterableFRS.Params<>(new LNormGenerator(M, Double.POSITIVE_INFINITY, problemBundle._normalizations));
        pFRS._samplingLimit = 1000000;
        pFRS._feasibleSamplesToGenerate = goals.length;
        pFRS._initialModels = new LNorm[goals.length];
        for (int i = 0; i < goals.length; i++)
            pFRS._initialModels[i] = new LNorm(goals[i].getParams()[0].clone(), goals[i].getParams()[1][0]);
        IterableFRS<LNorm> frs = new IterableFRS<>(pFRS);

        // Create the EA:
        IEMOD iemod = IEMOD.getIEMOD(0, false, false, R, goals,
                problemBundle, new Euclidean(), 10,
                iRule, referenceSetConstructor, dmFeedbackProvider, model, frs);
        ...
        if (generateScreenshots)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                String fp = path.toString() + File.separatorChar + "frs_screenshot_1_" + i;
        ...
        try
        {
            ...
            Thread.sleep(1000);
            frs.registerDecisionMakingContext(dmContext);

            if (!frs.initializeStep(report, preferenceInformation))
            {
                Thread.sleep(1000);
                ...
                for (int i = 0; i < pFRS._samplingLimit; i++)
                {
                    LNorm sampled = frs.executeStep(report, preferenceInformation);
                    if (report._models.size() == goals.length)
                    {
                            ...
                            (Double.compare(CA.calculateTheMostDiscriminativeCompatibilityWithValueModel(preferenceInformation, sampled), 0.0d) > 0))
                    {
                        ...
                        screenshot._barrier.await();
                        Path path = FileUtils.getPathRelatedToClass(AnimationFRS.class, "Projects", "src", File.separatorChar);
                        String fp = path.toString() + File.separatorChar + "frs_screenshot_2_1";
                        ImageSaver.saveImage(screenshot._image, fp, "jpg", 1.0f);
                        Thread.sleep(1000);
            ...
            frs.finalizeStep(report, preferenceInformation);
        ...
        try
        {
            ...
            if (generateScreenshots)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    screenshot._barrier.await();
                    Path path = FileUtils.getPathRelatedToClass(AnimationFRS.class, "Projects", "src", File.separatorChar);
                    String fp = path.toString() + File.separatorChar + "frs_screenshot_3_" + i;
                    ImageSaver.saveImage(screenshot._image, fp, "jpg", 1.0f);
                    Thread.sleep(1000);
                }
            }
