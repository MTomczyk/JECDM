...
/**
 * Wrapper for pymoo's NSGA-II implementation. Note that it attempts to access an external jar. The jar is not provided
 * as part of this framework. Its default executable method instantiates NSGA-II to solve a DTLZ2 algorithm. As for the
 * input (args[]), three parameters must be supplied: the number of objectives considered, the population size, and the
 * number of generations. The program returns the total execution time in ms (console output). IMPORTANT: If using your
 * own external py script, please provide a valid path to it in the class constructor.
 *
 * @author MTomczyk
 */
public class PYMOO extends AbstractEA implements IEA
{
    /**
     * Tests the wrapper.
     *
    ...
    public static void main(String[] args)
    {
        ...
        PYMOO pymoo = new PYMOO(M, populationSize, generations, new L32_X64_MIX(0));
        try
        {
            pymoo.init();
     ...
     * @param R              random number generator
     */
    public PYMOO(int M, int populationSize, int generations, IRandom R)
    {
        super(new Params("NSGAII_PYMOO", 0, R, true,
        ...
        String path = "D:" + File.separatorChar + "experiments" + File.separatorChar + "2025" + File.separatorChar
                + "SoftwareX_JECDM" + File.separatorChar + "pymoo_nsgaii.py";
        _processBuilder = new ProcessBuilder("python", path, String.valueOf(M),
     ...
     * The {@link PYMOO#step(EATimestamp)} method has no effect.
     *
