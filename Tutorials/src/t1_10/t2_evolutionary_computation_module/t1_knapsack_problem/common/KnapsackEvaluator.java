...
public class KnapsackEvaluator implements IEvaluate
{
     ...
     */
    public KnapsackEvaluator(Data data, boolean repairMode)
    {
        _data = data;
    ...
    public void evaluateSpecimens(ArrayList<Specimen> specimens) throws PhaseException
    {
        for (Specimen s : specimens)
        {
            ...
            if ((_repairMode) && (size > _data._capacity)) // if the repair mode is used
            {
                for (int index : _data._sortedIndices)
                {
                    ...
                    if (size <= _data._capacity) break; // break if the feasibility is restored
                }
            ...
            if ((!_repairMode) && (size > _data._capacity))// if the penalty mode is used
                s.setAuxScore(_data._capacity - size);
        }
    }
}
