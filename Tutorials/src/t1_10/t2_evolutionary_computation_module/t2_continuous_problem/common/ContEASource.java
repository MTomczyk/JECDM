...
public class ContEASource extends AbstractEASource implements IDataSource
{
    ...
    /**
     * If true (and the ``add generation'' flag is true), the specimen individual timestamp will be used instead of
     * the current timestamp's generation number.
     */
     ...
     * @param generationWhenConstructed if true (and the ``add generation'' flag is true), the specimen individual timestamp will be used instead of the current timestamp's generation number.
     */
    ...
    /**
     * Creates data point in the following form: [specimen first decision variable (x-coordinate); specimen performance value;
     * specimen second decision variable (y-coordinate).
     *
     * @return new data
     */
    @Override
    ...
    {
        ArrayList<Specimen> specimens = getDefaultSpecimens();
        double[][] data;
