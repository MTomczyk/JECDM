...
{
    /**
     * Creates a simple PCP 2D plot. It: <br>
                                                   ...
                                                   IPostPlotCreationAdjuster<ParallelCoordinatePlot2D> postPlotCreationAdjuster)
    {
        ...
        return getPlot(scheme, xLabel, yLabels, pDRM, yt, yf, fsr, schemeAdjuster,
     ...
     * @param yAxesTicksLabelsFormats string pattern for {@link DecimalFormat} used in ticks labels of Y-axes (each for
     *                                each Y-axis, 1:1 mapping; null if not used)
     * @param fsr                     rescaling factor used for all font-based objects
     * @return instantiated plot 2D
     */
    public static ParallelCoordinatePlot2D getPlot(String xLabel,
    ...
    public static ParallelCoordinatePlot2D getPlot(AbstractScheme scheme,
                                                   ...
                                                   IPostPlotCreationAdjuster<ParallelCoordinatePlot2D> postPlotCreationAdjuster)
    {
        NumberFormat[] decimalFormats = new NumberFormat[yAxesTicksLabelsFormats.length];
        for (int i = 0; i < yAxesTicksLabelsFormats.length; i++)
        {
            if (yAxesTicksLabelsFormats[i] == null) decimalFormats[i] = new DecimalFormat();
            else decimalFormats[i] = new DecimalFormat(yAxesTicksLabelsFormats[i]);
        }
        return getPlot(scheme, xLabel, yLabels, pDRM, yNoTicks, decimalFormats,
     ...
     * @param pDRM                     display ranges manager params container
     * @param yNoTicks                 adjusts the number of ticks for the Y-axes (each for each Y-axis, 1:1 mapping)
     * @param yAxesNumberFormat        y-axes number format (default {@link DecimalFormat} instance is used, if null;
     *                                 1:1 mapping)
     * @param fsr                      rescaling factor used for all font-based objects
     * @param schemeAdjuster           can be supplied to adjust the scheme on the fly (can be null); executed at the
     *                                 end of the customization of the plot params container (after all fields are set
    ...
    public static ParallelCoordinatePlot2D getPlot(String xLabel,
                                                   ...
                                                   DisplayRangesManager.Params pDRM,
                                                   int[] yNoTicks,
                                                   NumberFormat[] yAxesNumberFormat,
                                                   float fsr,
                                                   ISchemeAdjuster schemeAdjuster,
                                                   IPlotParamsAdjuster<ParallelCoordinatePlot2D.Params> plotParamsAdjuster,
                                                   IPostPlotCreationAdjuster<ParallelCoordinatePlot2D> postPlotCreationAdjuster)
    {
        return getPlot(WhiteScheme.getForPCP2D(), xLabel, yLabels, pDRM, yNoTicks, yAxesNumberFormat, fsr, schemeAdjuster,
                plotParamsAdjuster, postPlotCreationAdjuster);
    }
                                                   ...
                                                   IPostPlotCreationAdjuster<ParallelCoordinatePlot2D> postPlotCreationAdjuster)
    {
        ...
        AbstractFactory.adjustAxesPCP2D(pcp, yAxesNumberFormat, yNoTicks, yLabels.length);
