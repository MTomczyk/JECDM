...
/**
 * Tests basic functionalities related to parsing drawing labels.
 *
 * @author MTomczyk
 */
public class Test49_ParseLabels
{
    /**
    ...
    public static void main(String[] args)
    {
        ...
        // pP._debugMode = true;
        pP._title = "^TE$ST$ PL_OT";
       // pP._title = "TEST PLOT";
        pP._xAxisTitle = "X-ax^{is} $Test$ (j)";
        pP._yAxisTitle = "Y-Ax^is T_est (j)";
        pP._drawXAxis = true;
        pP._drawYAxis = true;
        pP._drawLegend = true;
        ...
        //pP._scheme._aligns.put(AlignFields.TITLE, Align.BOTTOM);
        ...
        ArrayList<IDataSet> dss = new ArrayList<>();
        {
            ...
            MarkerStyle ms = new MarkerStyle(1.0f, Gradient.getViridisGradient(100, false),
                    2, Marker.SQUARE);
            LineStyle ls = null;
            IDataSet ds = DataSet.getFor2D("^d_a$ta$", data, ms, ls);
            dss.add(ds);
        }

        pP._colorbar = new Colorbar(Gradient.getViridisGradient(10, false), "c^o$l$or^{bar}",
        ...
        plot.getComponentsContainer().getAxes()[0].setTickLabels(new String[]{"^A", "B$_2$", "$C$", "D_1", "EEE"});
